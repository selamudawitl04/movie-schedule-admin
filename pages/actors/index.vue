...
provide('deleteActorOrDirector', (id, numberOfMovies)=>{
    if(numberOfMovies > 0){ 
        ...
        const {mutate, onDone, loading, onError } = mutation(deleteActorMutation, 'admin');
        const variables = {
            id: id
        }
        mutate(variables)
...
<template>
    ...
    <div v-if="actors.length == 0 && !isloading" class=" bg-gray-dark p-6 text-center text-white font-bold">
        <p>No Actors Found </p>
    </div>
    <div>
