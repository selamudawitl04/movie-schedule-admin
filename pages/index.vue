...
function searchMovie(search) {
  ...
  onResult((result) => {
    ...
    if (type.value == "adminSearch") {
      ...
      const movie_actor = movies.value[0].movies_actors.find(
        (movie_actor) => movie_actor.actor.id == store.getActorSearch
      );
  ...
  <div>
    ...
    >
      <template v-slot:pagination>
        <label class="text-white bg-movie-gray-900 p-3" for="page">page</label>
        <select
          v-model="page"
          class="p-3 bg-movie-gray-900 bg-opacity-70 text-white"
          name="page"
          id="page"
        >
      ...
      <template v-slot:status>
        <label class="text-white bg-movie-gray-900 p-3" for="status"
          >status</label
        >
        <select
          v-model="status"
      ...
      <template v-slot:sort>
        <label class="text-white bg-movie-gray-900 p-3" for="sort">sort</label>
        <select
          v-model="order_by"
          class="p-3 bg-movie-gray-900 bg-opacity-70 text-white"
          name="sort"
