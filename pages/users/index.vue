...
<template>
  ...
  <div class="relative" v-if="users.length > 0 && !isloading">
    <LayoutSubHeader
      class="w-full"
      @refresh="refresh"
      :resultMessage="resultMeassage"
      name="Users"
    ></LayoutSubHeader>
    <div
      v-if="usersId.length > 0"
      class="absolute top-0 left-40 w-32 p-2 text-white font-bold rounded-md bg-movie-gray-900"
    >
      <button @click="displayForm = true">Send Message</button>
    </div>
    <div
      v-if="isMessageSent"
      class="absolute top-0 right-80 text-white font-bold rounded-md bg-movie-gray-900"
    >
      <h2 class="bg-movie-gray-900 text-white font-bold p-2">
    ...
    <div
      v-if="displayForm"
      class="absolute left-40 top-0 z-50 bg-white border border-gray-light rounded-md p-2 shadow-xl w-96"
    >
      ...
      <form @submit.prevent="handleSendMessage" action="">
        ...
        ></textarea>
        <div
          class="relative w-32 p-2 text-white font-bold rounded-lg bg-movie-gray-900"
        >
          <span
            v-if="sendLoading"
            a
            class="absolute left-12 top-1 animate-spin text-9xl inline-block w-8 h-8 border-[3px] border-current border-t-transparent text-primary-500 rounded-full"
            role="status"
            aria-label="loading"
          ></span>
