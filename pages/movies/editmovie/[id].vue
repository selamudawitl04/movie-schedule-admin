...
const handleEditMovie = () => {
  ...
  const newdate = Math.floor(
    new Date(movieVariable.data.date) / (1000 * 60 * 60 * 24)
  );
...
<template>
  ...
  <div
    v-if="!movieLoading && !serverError.error && movieData.director"
    class="custom-shadow w-full max-w-5xl flex justify-center px-6 mx-auto rounded-lg py-10"
  >
    <Form
      @submit="handleEditMovie"
    ...
    >
      ...
      <NuxtLink :to="'/'" class="text-movie-gray-900 py-4 font-bold"
        >back</NuxtLink
      >
        ...
        <div class="grid grid-cols-2 gap-6 w-full">
          ...
          <div class="">
            ...
            <Field
              ...
              type="text"
              class="border-2 p-2 border-gray rounded-md w-full focus:border-primary-500"
              placeholder="title"
          ...
          <DirectorsSet
            :selected-director="movieData.director.id"
      ...
      <button
        :disabled="loading"
        class="hover:bg-opacity-100 font-bold text-primary9 bg-primary-500 w-80 my-10 mx-auto block rounded-full p-2"
      >
        submit
      </button>
    </Form>
    <div
      v-if="displayForm"
      class="fixed top-20 z-50 bg-white border border-gray-light rounded-md p-2 shadow-xl w-96"
    >
      <div class="flex justify-between">
        <label
          class="bg-opacity-80 font-bold bg-movie-gray-900 text-white p-2 mb-4 rounded-md"
          for="message"
          >The movie date is changed please write reason below</label
        >
      </div>
      <form @submit.prevent="editWithReason" action="">
        <textarea
          v-model="message"
          required
          class="border border-gray text-primary9 rounded-md p-2"
          name="message"
          id="message"
        ...
        <div class="flex items-center justify-between">
          <div
            class="relative w-32 p-2 text-center text-primary9 font-bold rounded-lg bg-primary-500"
          >
          ...
          <div
            class="relative w-32 p-2 text-center text-primary9 font-bold rounded-lg bg-primary-500"
          >
            <button @click="cancelChange" type="button" class="text-center">
              cancel change
            </button>
          </div>
        </div>
