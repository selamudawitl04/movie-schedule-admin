...
onDone((result) => {
  ...
  contacts.value = contacts.value.filter(
    (contact) => contact.id != result.data.delete_contacts_by_pk.id
  );
...
const answer = ref("");
const {
  mutate: addFaqMutation,
  onDone: addFaqOnDone,
  onError: addFaqOnError,
} = mutation(addFaq);
addFaqOnDone((result) => {
  (alertRequired.value = true),
...
<template>
  <div class="flex mx-12 flex-row justify-start">
    <div class="w-1/2">
      ...
      <teleport to="body">
        <transition name="book">
          <div
            v-if="alertRequired"
            class="top-4 right-4 text-white font-bold bg-movie-gray-900 p-3 fixed"
          >
            <p>{{ alertMessage }}</p>
          </div>
        </transition>
    ...
    <div class="pt-0 ml-16 w-1/3">
      <h3 class="text-2xl font-bold pb-2 text-primary9 text-center">
      ...
      <form @submit.prevent="handelAddFaq" action="#" class="">
        <div class="mb-2">
          ...
          <input
            ...
            type="text"
            id="subject"
            class="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light"
        ...
        <div class="sm:col-span-2">
          ...
          <textarea
            ...
            id="message"
            rows="6"
            class="block p-2.5 w-full text-gray-900 text-sm text-primary9 bg-gray-50 rounded-lg shadow-sm border border-gray focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
            placeholder="write answer..."
          ></textarea>
        </div>
        <button
          type="submit"
          class="py-3 my-3 px-5 bg-opacity-50 hover:bg-opacity-90 text-center rounded-lg bg-primary-500 sm:w-full text-primary4 font-bold"
        >
          ADD
        </button>
      </form>
