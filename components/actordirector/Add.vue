            ...
            <div class=" flex items-center justify-center">
                ...
                <div class=" flex flex-col space-y-6">
                    <div>
                      ...
                      <Field name="first_name"  v-model="item.first_name" type="text" class=" py-1"  placeholder="first name" :class="{ 'border-red': errors.title }" />
                          <transition name="error">
                            <p class="text-red text-sm" >{{errors.first_name}}</p>
                          </transition>
                    </div>
                    <div>
                      <Field name="last_name"  v-model="item.last_name" type="text" class=" py-1"  placeholder="last name" :class="{ 'border-red': errors.title }" />
                          <transition name="error">
                            <p class="text-red text-sm" >{{errors.last_name}}</p>
                          </transition>
                    </div>
                    <button :disabled="isloading" class=" font-bold text-white rounded-sm bg-yellow-bright w-full p-2">Submit</button>
                </div>
            </div>
        </Form>
    </div>
