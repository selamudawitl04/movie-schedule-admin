...
function setSearch() {
  ...
  } else [store.setDirectorSearch(props.item.id)];
  router.push("/");
}
...
<template>
  <tr>
    ...
    >
      {{ item.no_of_movie || 0 }} movies
    </td>
    <td class="relative text-right">
      <div
        class="absolute left-5 top-6 px-4 text-center text-white font-bold bg-movie-gray-900 rounded-md"
      >
    ...
    <td class="relative text-right">
      <div
        class="px-4 text-center absolute left-4 top-6 text-white font-bold bg-movie-gray-900 rounded-md"
      >
