...
const directorFullName = computed(
  () => movie.value.director.first_name + " " + movie.value.director.last_name
);
const date = computed(() => {
  ...
  <tr>
    <td class="text-movie-gray-900 font-bold">
      <NuxtLink :to="detailPageLink">
        <img
          class="w-8 rounded-md h-8 inline"
          :src="imgUrl"
          alt="Movie Image"
        />
        {{ movie.title }}
    ...
    <td class="text-movie-gray-900 font-bold">{{ movie.duration }}</td>
    <td class="relative text-primary9 font-bold">
      ...
      <span
        v-if="loading"
        class="absolute left-7 top-6 animate-spin text-9xl inline-block w-8 h-8 border-[3px] border-current border-t-transparent text-primary-500 rounded-full"
        role="status"
        aria-label="loading"
      ></span>
    ...
    <td class=" ">
      <NuxtLink
        :to="editPageLink"
        class="py-1 ml-0.5 text-white font-bold bg-movie-gray-900 rounded-md px-4"
        >Edit</NuxtLink
      >
    </td>
    <td class="relative">
      <button
        @click="toggleDelete"
        class="px-1.5 ml-0.5 pt-0.5 text-white font-bold bg-movie-gray-900 rounded-md"
      >
        Delete
      </button>
      <span
        v-if="deleteLoading"
        a
        class="absolute left-9 top-6 animate-spin text-9xl inline-block w-8 h-8 border-[3px] border-current border-t-transparent text-primary-500 rounded-full"
        role="status"
        aria-label="loading"
      ></span>

      <BasePopup v-if="showDeleteDialog">
        <div class="w-96 rounded-md bg-movie-gray-900 absolute">
          <div class="relative">
            <h3
              class="rounded-md text-2xl bg-primary4 p-4 text-center text-white font-bold"
            >
              Delete Movie
            </h3>
            <p
              class="absolute top-4 right-0 px-4"
          ...
          <p class="px-4 py-6 text-white text-10">
            Are you sure you want to delete this movie? if you sure change
            status of movie to closed and then delete it
          </p>
          ...
          <div class="rounded-md text-center">
            <button
              @click="deleteMovie"
              class="w-full bg-primary-500 py-4 text-white font-bold"
            >
              Delete
            </button>
          </div>
